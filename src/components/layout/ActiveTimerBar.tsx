...
export default function ActiveTimerBar() {
  const timer = useDataStore((s) => s.activeTimer); const projects = useDataStore((s) => s.projects) || []; const cache = useDataStore((s) => s.cache); const projectTimeTypes = useDataStore((s) => s.projectTimeTypes); const stopTimer = useDataStore((s) => s.stopTimer); const [now, setNow] = useState(Date.now());
