...
function TaskProgressEntryEditor({ entry, onSave, onCancel }: { entry?: TaskProgressEntry; onSave: (entry: TaskProgressEntry) => void; onCancel: () => void }) {
  ...
  return <div className="task-progress-editor"><div className="btn-row"><button type="button" className="btn small" disabled={!titel.trim()} onClick={save}>Eintrag übernehmen</button><button type="button" className="btn secondary small" onClick={onCancel}>Abbrechen</button></div><div className="field-grid"><div className="field"><label>Datum</label><input type="date" value={datum} onChange={(event) => setDatum(event.target.value)} /></div><div className="field"><label>Titel</label><input value={titel} onChange={(event) => setTitel(event.target.value)} placeholder="Was hat sich geändert?" /></div></div><RtfField value={content} onChange={setContent} title="Verlaufseintrag" placeholder="Änderung oder Zwischenstand beschreiben…" /></div>;
}
