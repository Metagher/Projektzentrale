  ...
  async function scan() {
    ...
    for (const project of projects) {
      ...
      data.tasks.forEach((task) => {
        ...
        const validAppointments = (task.termine || []).filter(validDate);
        if (validAppointments.length !== (task.termine || []).length) add({ ...prefix, area: 'Aufgabe', problem: 'Mindestens ein Termin enthält kein gültiges Datum', detail: task.titel, actionLabel: 'Bereinigen', run: () => store.saveTask(project.id, { ...task, termine: validAppointments }) });
