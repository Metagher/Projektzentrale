...
export default function ZeitenTab({ project, data }: { project: Project; data: ProjectCache }) {
  ...
  const billedDays = new Map<string, number>();
  const billedItems: NonNullable<BilledTimeRow['items']> = [];
  abrechnungen.forEach((item) => { if (item.minutes > 0 && item.datum) { billedDays.set(item.datum, (billedDays.get(item.datum) || 0) + item.minutes); billedItems.push({ date: item.datum, art: item.art, label: item.bemerkung || item.kunde, minutes: item.minutes }); } });
  const billedMinutesTotal = abrechnungen.reduce((sum, item) => sum + item.minutes, 0);
  ...
  return <section className="time-tab"><div className="module-section-head"><div><span className="eyebrow">Zeiterfassung</span><h3>{formatDuration(total)} Gesamtzeit</h3><p>Projektzeit wird nach den global festgelegten Zeittypen und Aufgaben ausgewiesen.</p></div><button className="btn small" onClick={() => setManual(true)}>+ Zeit nachtragen</button></div>
    ...
    <TimeAnalyticsOverview entries={entries} projects={[project]} workdayOverrides={workdayOverrides} heading="Projektzeit nach Tag und Kalenderwoche" billedRows={[{ projectId: project.id, minutes: billedMinutesTotal, days: Array.from(billedDays, ([date, minutes]) => ({ date, minutes })), items: billedItems }]} taskLabels={Object.fromEntries(data.tasks.map((task) => [task.id, `${task.nr} · ${task.titel}`]))} taskCategories={Object.fromEntries(data.tasks.map((task) => [task.id, task.kategorie || 'Sonstiges']))} timeTypeLabels={Object.fromEntries(projectTimeTypes.map((type) => [type.id, type.name]))} onSaveEntry={saveTimeEntry} onDeleteEntry={deleteTimeEntry} />
