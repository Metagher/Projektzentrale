...
export default function ZeitenTab({ project, data }: { project: Project; data: ProjectCache }) {
  ...
  data.tasks.forEach((task) => { const minutes = Number(task.billedMinutes) || 0; if (minutes > 0 && task.billedDate) { const day = billedDays.get(task.billedDate) || { taskMinutes: 0, communicationMinutes: 0 }; day.taskMinutes += minutes; billedDays.set(task.billedDate, day); billedItems.push({ date: task.billedDate, kind: 'Aufgabe', label: `${task.nr} · ${task.titel}`, minutes }); } });
  data.comms.forEach((comm) => { const minutes = Number(comm.billedMinutes) || 0; if (minutes > 0 && comm.datum) { const day = billedDays.get(comm.datum) || { taskMinutes: 0, communicationMinutes: 0 }; day.communicationMinutes += minutes; billedDays.set(comm.datum, day); billedItems.push({ date: comm.datum, kind: 'Kommunikation', label: comm.betreff || comm.kanal, minutes }); } });
