...
function CurrentStateEditor({ entry, onSave, onClose }: { entry: DocEntryValue; onSave: (value: DocEntryValue) => Promise<void>; onClose: () => void }) {
  ...
  return <section className="doc-section current-state-editor"><div className="btn-row"><button className="btn small" onClick={save}>Projektstand speichern</button><button className="btn secondary small" onClick={onClose}>Abbrechen</button></div><RtfField value={content} onChange={setContent} title="Aktueller Projektstand" placeholder="Was ist umgesetzt? Was wurde entschieden? Was ist kritisch? Was sind die nächsten Schritte?" /><AfnChipsField value={afns} onChange={setAfns} /></section>;
}
