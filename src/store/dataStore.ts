...
export const useDataStore = create<DataStoreState>((set, get) => ({
  ...
  assignAbrechnungenToProject: async (ids, projectId) => {
    const idSet = new Set(ids);
    const abrechnungen = get().abrechnungen.map((item) => idSet.has(item.id) ? { ...item, projectId } : item);
    set({ abrechnungen });
    await sSet(client(), 'abrechnungen', abrechnungen);
  },
  ...
  stopTimer: async () => {
    ...
    if (!timer.taskId && get().timeEntryReviewEnabled) {
      ...
      const result = await useModalStore.getState().timeEntryReview({ startedAt: timer.startedAt, endedAt, assignmentLabel, timeTypes: get().projectTimeTypes, initialTimeTypeId: timer.timeTypeId, contacts: data.contacts, tasks: data.tasks });
      if (!result) return;
      const durationMinutes = Math.max(1, Math.round((Date.parse(result.endedAt) - Date.parse(result.startedAt)) / 60000));
      const entry: TimeEntry = { id: uid(), projectId: timer.projectId, taskId: result.taskId || timer.taskId, timeTypeId: result.taskId ? undefined : result.timeTypeId, timeTypeName: result.taskId ? undefined : result.timeTypeName, kontaktIds: result.kontaktIds, startedAt: result.startedAt, endedAt: result.endedAt, durationMinutes, note: result.note, createdAt: endedAt };
