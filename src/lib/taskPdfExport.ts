...
export function exportTaskToPdf(project: Project, task: Task, contact?: Contact) {
  ...
  const tags = doc.createElement('div'); tags.className = 'tags'; if (task.teilprojekt) addText(tags, 'span', `Teilprojekt: ${task.teilprojekt}`, 'tag'); (task.afns || []).forEach((afn) => addText(tags, 'span', `AFN ${afn}`, 'tag')); if (task.wartetAuf) addText(tags, 'span', `Wartet auf: ${task.wartetAuf}`, 'tag'); if (task.wartetSeit) addText(tags, 'span', `Wartet seit: ${fmtDate(task.wartetSeit)}`, 'tag'); if (tags.childNodes.length) main.appendChild(tags);
