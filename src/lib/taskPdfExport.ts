...
export function exportTaskToPdf(project: Project, task: Task, contact?: Contact) {
  ...
  [['Aufgabennummer', `#${task.nr || '—'}`], ['Status', task.status], ['Fällig am', fmtDate(task.faelligAm)], ['Termine', task.termine?.length ? task.termine.map(fmtDate).join(', ') : '—'], ['Ansprechpartner', contact?.name || '—'], ['Erstellt am', fmtDate(task.erstelltAm?.slice(0, 10))], ['Abgeschlossen am', task.abgeschlossenAm ? fmtDate(task.abgeschlossenAm.slice(0, 10)) : '—']].forEach(([label, value]) => { const box = doc.createElement('div'); box.className = 'meta'; addText(box, 'span', label); addText(box, 'strong', value); meta.appendChild(box); });
