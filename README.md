...
Zugriff auf die Daten, selbst wenn URL und anon key bekannt werden. Der Login läuft über
E-Mail + Passwort — es wird dabei keine Mail verschickt, also auch kein Rate-Limit oder
SMTP-Thema.

1. Unter **Authentication → Sign In / Providers → Email** bzw. **Authentication → Settings**:
...
2. Unter **Authentication → Users → Add user** deinen eigenen Account manuell anlegen: deine
   E-Mail-Adresse und ein **echtes, starkes Passwort** (das ist jetzt das tatsächliche
   Zugangs-Credential) — markiere den Nutzer dabei direkt als **"Auto Confirm User"**.
   Nicht über "Invite user" anlegen, das schickt unnötig eine Mail.
3. In der App beim Login E-Mail-Adresse und dieses Passwort eintragen.
